...
def remove_augmentation():
    ...
    if len(os.list_dir('/home/dell/Documentos/rn-cin-covid/data/tmp/')) > 0:
    ...
    for image in os.list_dir('data/train/positive'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(temp_dir_train, 'positive'))
            
    for image in os.list_dir('data/train/negative'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(temp_dir_train, 'negative'))
            
    for image in os.list_dir('data/val/positive'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(temp_dir_val, 'positive'))
    
    for image in os.list_dir('data/val/negative'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(temp_dir_val, 'negative'))
...
def add_back_augmentation():
    ...
    if len(os.list_dir('/home/dell/Documentos/rn-cin-covid/data/tmp/')) < 1:
    ...
    for image in os.list_dir('data/tmp/train/positive'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(dir_train, 'positive'))
            
    for image in os.list_dir('data/tmp/train/negative'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(dir_train, 'negative'))
            
    for image in os.list_dir('data/tmp/val/positive'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(tdir_val, 'positive'))
    
    for image in os.list_dir('data/tmp/val/negative'):
        if image in train_labels_gauss['image'].tolist():
            shutil.copy(image, os.path.join(dir_val, 'negative'))
